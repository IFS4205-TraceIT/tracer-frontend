...
async function onSubmit() {
    try {
        errorMessage.value = "";
        loading.value = true;
        const { data, error } = await useApi(`/building/genqrcode/${buildingname.value}`)
        loading.value = false;
        if (error.value) {
...
<template>
    <div class="container flex w-full mx-auto my-4">
        <div class="overflow-x-auto w-full relative shadow-md sm:rounded-lg">
            <div class="p-5 text-left text-gray-900 bg-white dark:text-white dark:bg-gray-800">
                <h3 class="text-lg font-semibold">Get QRCode for buildings here!</h3>
                <form class="flex flex-col gap-4 my-4 w-full" @submit.prevent="onSubmit">
                    <div v-if="!!errorMessage" class="p-4 text-red-700 border rounded border-red-900/10 bg-red-50"
                        role="alert">
                        <strong class="text-sm font-medium">{{ errorMessage }}</strong>
                    </div>
                    <div>
                        <input id="building" v-model="buildingname" :disabled="loading" name="buildingname"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            rows="10" required autofocus />
                    </div>
                ...
                <img class="mx-auto" v-if="buildingimage !== ''" :src="buildingimage" alt="Bad QRCode :(" />
            </div>
        </div>
    </div>
</template>
