    ...
    <div class="overflow-x-auto w-full relative shadow-md sm:rounded-lg">
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <caption class="p-5 text-lg font-semibold text-left text-gray-900 bg-white dark:text-white dark:bg-gray-800">
          ...
          <br />
          <h3 v-if="buildinglogs && (buildinglogs as Array<any>).length !== 0" class="text-base"> Building:
            {{buildinglogs[0].building.name}}</h3>
          ...
          <div class="py-4">
            <div class="relative mt-1">
              ...
              <input id="table-search" type="date"
                class="block p-2 pl-10 w-80 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Search for items" @change="filterData" />
            </div>
          </div>
        </caption>
        ...
        <tbody>
          <tr v-for="log in buildinglogs" :key="log.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
      ...
      <div v-if="buildinglogs && (buildinglogs as Array<any>).length === 0" class="text-center p-4 dark:bg-gray-800">
        No records found
      </div>
    </div>
  </div>
